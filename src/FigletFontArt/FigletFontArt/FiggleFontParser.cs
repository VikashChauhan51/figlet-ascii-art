...
public static class FiggleFontParser
{
    ...
    public static FiggleFont Parse(Stream stream, StringPool? pool = null)
    {
        ...
        if (firstLine == null)
            throw new ArgumentNullException("Font file is empty.");

        var match = _firstLinePattern.Match(firstLine);

        if (!match.Success)
            throw new ArgumentNullException("Font file has invalid first line.");
        ...
        */
        ...
        FiggleCharacter ReadCharacter()
        {
            ...
            for (var i = 0; i < height; i++)
            {
                ...
                if (line == null)
                    throw new ArgumentNullException("Unexpected EOF in Font file.");
        ...
        readLine:
            ...
            if (!ParseUtil.TryParse(line, out var code))
                throw new ArgumentNullException($"Unsupported code-tagged character code string \"{line}\".");
