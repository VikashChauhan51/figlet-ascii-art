...
public static class FiggleFonts
{
    ...
    {
        using var stream = typeof(FiggleFonts).GetTypeInfo().Assembly.GetManifestResourceStream("FigletFontArt.Fonts.zip");
        using var zip = new ZipArchive(stream, ZipArchiveMode.Read);

        var entry = zip.GetEntry(name + ".flf");

        if (entry == null)
            throw new ArgumentNullException($"No embedded font exists with name \"{name}\".");
